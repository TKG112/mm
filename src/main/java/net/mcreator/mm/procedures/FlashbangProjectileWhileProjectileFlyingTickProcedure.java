...
public class FlashbangProjectileWhileProjectileFlyingTickProcedure {
        ...
        if (world instanceof ServerLevel) {
            ...
            if (entity instanceof Player player) {
                ...
                if (distance < 16) { // Only apply the effect if the player is within 16 blocks
                    ...
                    playEarRingingSound(player, 16 - (int) distance); // Play the corresponding ear-ringing sound
    ...
    private static void playEarRingingSound(Player player, int soundIndex) {
        if (player.level().isClientSide) {
            String soundName = "mm:ear-ringing-" + soundIndex;
            player.playSound(ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(soundName)), 1.0F, 1.0F);
        }
    }
