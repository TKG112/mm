...
public class TanGPNVGItem extends Item implements GeoItem, ICurioItem {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	public String animationprocedure = "Idle";

	// Animations
	private static final RawAnimation ANIM_IDLE = RawAnimation.begin().thenLoop("animation.tan_gpnvgs.idle");
	private static final RawAnimation ANIM_OPEN = RawAnimation.begin().thenPlayAndHold("animation.tan_gpnvgs.opening").thenLoop("animation.tan_gpnvgs.opened");
	private static final RawAnimation ANIM_CLOSE = RawAnimation.begin().thenPlayAndHold("animation.tan_gpnvgs.closing").thenLoop("animation.tan_gpnvgs.closed");
		...
		consumer.accept(new IClientItemExtensions() {
			...
			public HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
				if (this.renderer == null)
					this.renderer = new TanGPNVGRenderer();
