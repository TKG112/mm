...
public class BlackPlateCarrierInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == MmModItems.BLACK_PLATE_CARRIER.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(4) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != MmModItems.BLACK_PLATE_CARRIER.get();
			}
