...
public class TanPlateCarrierPouchesInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == MmModItems.TAN_PLATE_CARRIER_POUCHES.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(8) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != MmModItems.TAN_PLATE_CARRIER_POUCHES.get();
			}
