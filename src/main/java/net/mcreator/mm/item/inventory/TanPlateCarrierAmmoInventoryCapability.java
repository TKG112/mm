...
public class TanPlateCarrierAmmoInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == MmModItems.TAN_PLATE_CARRIER_AMMO.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(4) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != MmModItems.TAN_PLATE_CARRIER_AMMO.get();
			}
