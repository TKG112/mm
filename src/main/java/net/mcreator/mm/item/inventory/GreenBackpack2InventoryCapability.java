...
public class GreenBackpack2InventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == MmModItems.GREEN_BACKPACK_2.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(15) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != MmModItems.GREEN_BACKPACK_2.get();
			}
