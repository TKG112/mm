...
public class ItemInteractionEvent {
    ...
    public static void onRightClickItem(LivingEntityUseItemEvent event) {
        ...
        if (GenericNVGGogglesItem.isNVGOnFace(stack) && !CommonConfig.CAN_EAT_WITH_FACEWEAR_DOWN.get()) {
            event.setCanceled(true);
        }
        ModernMayhemMod.LOGGER.info("Item is edible: {}, player can eat with facewear down: {}, nvg on face: {}", event.getItem().isEdible(), CommonConfig.CAN_EAT_WITH_FACEWEAR_DOWN.get(), GenericNVGGogglesItem.isNVGOnFace(stack));
    }
}
